...
def main(
    ...
    quantized_path: Path = Path("checkpoints/meta-Transformer/Transformer-2-7b-chat-hf/model.pth"),
    ...
    if quantized_path:
        quant_model = GPTQModel.from_quantized(
            quantized_path,
            backend=get_backend('BITBLAS'),
        )
        import copy
        for layer, quant_layer in zip(model.layers, quant_model.model.model.layers):
            layer.attention.wq = copy.deepcopy(quant_layer.self_attn.q_proj)
            layer.attention.wk = copy.deepcopy(quant_layer.self_attn.k_proj)
            layer.attention.wv = copy.deepcopy(quant_layer.self_attn.v_proj)
            layer.attention.wo = copy.deepcopy(quant_layer.self_attn.o_proj)
            layer.feed_forward.w1 = copy.deepcopy(quant_layer.mlp.gate_proj)
            layer.feed_forward.w3 = copy.deepcopy(quant_layer.mlp.up_proj)
            layer.feed_forward.w2 = copy.deepcopy(quant_layer.mlp.down_proj)
        del quant_model
        # import code; code.interact('quant_model', local=dict(globals(), **locals()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quantized_path', type=Path, default=Path("../../../checkpoints/meta-llama/Llama-2-7b-chat-hf/model.pth"), help='Qauntized model checkpoint path generated by GPTQModel.')
    ...
    main(
        ...
        args.temperature, args.checkpoint_path, args.quantized_path, args.quantization, args.kv_cache_quantization, args.cache_size, args.linear_causal_mask, args.save, args.compile, args.compile_prefill, args.profile, args.memory_profile, args.device, args.precision, args.write_result
    )
