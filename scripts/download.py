...
def hf_download(repo_id: Optional[str] = None, hf_token: Optional[str] = None) -> None:
    ...
    os.makedirs(f"/SSD/JG/checkpoints/{repo_id}", exist_ok=True)
    try:
        snapshot_download(repo_id, local_dir=f"/SSD/JG/checkpoints/{repo_id}", local_dir_use_symlinks=False, token=hf_token)
