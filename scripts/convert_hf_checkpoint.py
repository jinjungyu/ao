...
def convert_hf_checkpoint(
    *,
    ...
    if '68m' in model_name:
        model_json_safetensors = checkpoint_dir / 'model.safetensors'
        model_json_pytorch = checkpoint_dir / 'model.bin'
        model_json = None
        try:
            assert model_json_safetensors.is_file()
            model_json = model_json_safetensors
            print(f"Found safetensors at {model_json_safetensors}")
        except AssertionError:
            print(f"{model_json_safetensors} not found")
        if model_json is None:
            try:
                assert model_json_pytorch.is_file()
                model_json = model_json_pytorch
                print(f"Found pytorch at {model_json_pytorch}")
            except AssertionError:
                print(f"{model_json_pytorch} not found")
    
        if model_json is None: raise Exception("No model found!")
        bin_files = {model_json}
    else:
    ...
    if fused:
        ...
        print(f"Saving checkpoint to {checkpoint_dir / 'model_fused.pth'}")
        torch.save(final_result, checkpoint_dir / "model_fused.pth")
    else:
